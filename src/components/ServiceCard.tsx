  ...
  return (
    <Card className={cn("h-full transition-all duration-200 hover:shadow-lg border-t-4 border-t-toledo-gold", className)}>
      <CardHeader>
        ...
        <CardTitle className="text-lg font-bold text-toledo-blue">{title}</CardTitle>
        <CardDescription className="text-sm text-toledo-grey mt-3 whitespace-normal">{description}</CardDescription>
      </CardHeader>
      <CardContent>
        <ul className="space-y-2">
          {features.map((feature, index) => (
            <li key={index} className="flex items-start">
              ...
              <span className="text-sm text-toledo-grey whitespace-normal">{feature}</span>
            </li>
          ))}
        </ul>
      </CardContent>
