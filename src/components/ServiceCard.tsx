  ...
  return (
    <Card className={cn("h-full transition-all duration-200 hover:shadow-lg border-t-4 border-t-toledo-gold", className)}>
      <CardHeader>
        ...
        <CardDescription className="text-toledo-grey mt-3 text-justify hyphens-auto no-orphans">{description}</CardDescription>
      </CardHeader>
      <CardContent>
        <ul className="space-y-2">
          {features.map((feature, index) => (
            <li key={index} className="flex items-start">
              ...
              <span className="text-toledo-grey text-justify hyphens-auto no-orphans">{feature}</span>
            </li>
          ))}
        </ul>
      </CardContent>
